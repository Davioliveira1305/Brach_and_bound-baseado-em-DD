  ...
  if metodo != 4:
    ...
    # Se os nós são iguais a heurística é aplicada
      ...
      while True:
        ...
        i += 1
      j = 0
      # Guardando os nós com valores de FO iguais em uma lista(movendo à direita)
      while True:
        if camada[w - 1].valor == camada[w + j].valor:
          iguais.append(camada[w + j])
          j += 1
        else: break
      # Guardando os nós com valores de FO diferentes em uma lista(A partir da borda)
      ...
      for no in (diferentes + iguais):
        camada.remove(no)
      # Mesclagem dos nós 
      estado_1, estado_2 = set().union(*[no.estado for no in iguais]), set().union(*[no.estado for no in diferentes])
      valor_1, valor_2 = iguais[0].valor, diferentes[0].valor
      solucao_1, solucao_2 = iguais[0].solucao, diferentes[0].solucao
      ordenacao_merge_1, ordenacao_merge_2 = iguais[0].ordenacao, diferentes[0].ordenacao
