...
def branch_and_bound_dd(instancia, w_relax, w_restrito, metodo_relax, metodo_restrito):
  # Leitura dos dados da instância
  dados = leitura(instancia)
  # Nó inicial da árvore de branch and bound
  no_inicial = Node(set(dados[0]), 0, [0 for _ in range(len(dados[0]))], 0)
  ...
  while len(abertos) != 0:
    ...
    sol_relax, cut_set_exato, se_dd_exato = dd_relaxado(no, dados, w_relax, metodo_relax)
    ...
    sol_restrita = dd_restrito(no, dados, w_restrito, metodo_restrito)
